...
V3 = "extract_ontology.v3.md"   # 현재 활성 버전
...
def test_v2_is_preserved():
    """v2 도 실제 추출 2건에 쓰였다. `요약` 규칙은 v3 에만 있어야 한다 (D-027)."""
    v2 = load_prompt(V2)
    assert "영어 표준형" in v2.system      # v2 의 규칙은 그대로
    assert "### 요약" not in v2.system     # v3 의 규칙이 새어들지 않았다


def test_default_prompt_is_v3():
...
# D-020: 자유 태그 영어 표준형
# ---------------------------------------------------------------------------
def test_v3_states_english_first_rule():
    system = load_prompt(V3).system
    assert "영어 표준형" in system
    assert "병기하지 않는다" in system


@pytest.mark.parametrize("good", ["Fuzzing", "Vibe Coding", "Transformer"])
def test_v3_shows_good_examples(good):
    assert good in load_prompt(V3).system


@pytest.mark.parametrize("bad", ["퍼징(Fuzzing)", "바이브코딩", "트랜스포머"])
def test_v3_shows_bad_examples(bad):
    """실제로 나왔던 표기를 반례로 명시해야 한다."""
    assert bad in load_prompt(V3).system


# ---------------------------------------------------------------------------
# D-022: Agent 판단 기준
# ---------------------------------------------------------------------------
def test_v3_narrows_agent_criteria():
    system = load_prompt(V3).system
...
# D-021: 발표유형 신규 값
# ---------------------------------------------------------------------------
def test_v3_explains_community_discussion_boundary():
    system = load_prompt(V3).system
...
def test_release_type_list_injected_includes_new_value(item):
    ...
    system, _ = load_prompt(V3).render(**build_variables(item))
...
# 렌더링
# ---------------------------------------------------------------------------
def test_v3_renders_without_leftover_placeholders(item):
    system, user = load_prompt(V3).render(**build_variables(item))
...
def test_v3_renders_with_empty_body(item):
    """본문 없는 피드(title_only)에서도 빈 자리가 남지 않아야 한다."""
    _, user = load_prompt(V3).render(**build_variables(item.model_copy(update={"body": ""})))
    assert "{{" not in user
    assert "(정보 없음)" in user


def test_v3_keeps_earlier_core_rules():
    """새 규칙을 넣다가 기존 규칙을 떨어뜨리지 않았는지."""
    v1, v3 = load_prompt(V1).system, load_prompt(V3).system
    for rule in [
        "주어진 원문에만 근거한다",
        "통제어휘 밖의 값을 만들지 않는다",
        "원문이 잘려 있을 수 있다",
        ...
        assert rule in v3, f"v3 에서 '{rule}' 규칙이 사라졌다"


# ---------------------------------------------------------------------------
# D-027: 요약 필드 (v3 신규)
# ---------------------------------------------------------------------------
def test_v3_states_summary_rules():
...
def test_v3_forbids_meta_expressions_in_summary():
    system = load_prompt(V3).system
