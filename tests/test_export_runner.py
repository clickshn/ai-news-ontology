...
class TestUrlInjection:
    ...
    # 나머지 주입 테스트(빈 응답 재시도 / 입력 순서 / 타임아웃)는
    # `tests/test_export_urls.py` 에 있다. arXiv 요청 경로를 목으로 덮어야 하는데,
    # 그 목이 여기 있으면 **이 파일이 전송 계층까지 재게 된다** — session-06 F13
    # 수정으로 전송이 `collectors.rss.fetch_feed_bytes` 로 합쳐지면서 갈랐다.
