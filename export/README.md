...
### 응답은 변환보다 **먼저** 디스크로 간다 (D-052 / D-077)

`replay_one` 은 `parse_into` 를 감싼 **`finally`** 에서 `raw/` 를 쓴다. 순서가
규칙이고, 순서를 지키는 것이 구조여야 한다 — 주석으로만 두면 다음 사람이 그 줄을
다시 아래로 옮긴다. **실제로 그렇게 돼 있었고**(행을 다 만든 뒤에 썼다), F5 테스트를
쓰다가 드러났다.

| 나가는 경로 | 응답이 남는가 |
|---|---|
| 성공 (행 변환이 터져도) | ✅ — 잃는 것은 돈이 아니라 **그 모델의 그 응답**이다 |
| 격리 (`SchemaMismatchError`) | ✅ — **왜 못 지켰는지**가 원본에만 있다 |
| 전송 실패 (응답 0건) | 파일을 만들지 않는다 — 빈 파일은 다음 실행이 **결과로 읽는다** |

같은 이유로 `rows.json` 은 **매 건마다** 갱신되고, 0건 실행에도 만들어진다
(D-078) — 실행 디렉터리의 구성이 건수에 따라 달라지지 않는다.
