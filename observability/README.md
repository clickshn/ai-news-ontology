...
> ⚠️ **단가는 단계가 아니라 모델에 붙는다** (D-076). 위 표는 *벤더 실행* 관측치라
> 단계와 모델이 1:1 이지만, 이전(ADR-018) 이후 두 단계는 `gemma-4-31B-it` 로 돌고
> **토큰당 청구가 없다.** 코드에서 곱하는 값의 정본은 `export/usage.py: MODEL_PRICES`
> 이고 **모델명으로** 찾는다. 이 표를 고치면 거기도 같이 고친다 — 갈리면 어느 쪽이
> 맞는지 알 수 없다. 아래 시나리오는 **벤더로 되돌렸을 때**의 계산이다.
